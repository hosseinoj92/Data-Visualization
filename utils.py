import pandas as pd
import csv
import os
from PyQt5.QtWidgets import QMessageBox  # Uncomment if using PyQt5

def read_numeric_data(file_path, parent=None, delimiter='\t', max_lines=100):
    """
    Reads a CSV or text file, skips metadata, detects headers and numeric data dynamically.

    Parameters:
        file_path (str): Path to the CSV file.
        parent (QWidget): Parent widget for QMessageBox.
        delimiter (str): Delimiter used in the file (default is tab).
        max_lines (int): Maximum number of lines to search for numeric data.

    Returns:
        tuple: (df, x, y) where df is the cleaned DataFrame, and x, y are numpy arrays.
               Returns (None, None, None) if reading fails.
    """
    header_row_index = None
    data_row_index = None

    try:
        # Read all lines from the file
        with open(file_path, 'r') as f:
            lines = f.readlines()

        # Iterate through the lines to find the header and data rows
        for i, line in enumerate(lines):
            # Limit the number of lines to search to avoid long processing times
            if i >= max_lines:
                break

            # Strip whitespace and split the line using the delimiter
            row = line.strip().split(delimiter)
            # Remove empty strings
            row = [cell.strip() for cell in row if cell.strip()]

            # Skip empty or whitespace-only lines
            if not row:
                continue

            if is_numeric_row(row):
                data_row_index = i
                break
            else:
                header_row_index = i  # Potential header row

        # If no numeric data found, raise an error
        if data_row_index is None:
            raise ValueError("Could not detect numeric data.")

        # Prepare the list of rows to skip
        skiprows = [i for i in range(data_row_index) if i != header_row_index]

        # Read the data, skip the skiprows, and set header=None
        df = pd.read_csv(
            file_path,
            delimiter=delimiter,
            skiprows=skiprows,
            header=None,
            engine='python'
        )

        if header_row_index is not None:
            # Read the header line to get the column names
            header_line = lines[header_row_index].strip()
            columns = [cell.strip() for cell in header_line.split(delimiter) if cell.strip()]
            # Assign the columns
            df.columns = columns
        else:
            # No header detected
            pass  # Keep default column names

        # Convert all columns to numeric where possible
        df = df.apply(pd.to_numeric, errors='coerce')

        # Drop rows where all values are NaN
        df.dropna(how='all', inplace=True)

        # Ensure there are at least two columns for X and Y
        if df.shape[1] < 2:
            raise ValueError("Not enough numeric columns for plotting.")

        # Extract X and Y columns
        x = df.iloc[:, 0].values  # X-axis data
        y = df.iloc[:, 1].values  # Y-axis data

        return df, x, y

    except ValueError as e:
        if parent:
            QMessageBox.warning(parent, "Data Read Error", str(e))
        else:
            print(f"Data Read Error: {e}")
        return None, None, None
    except Exception as e:
        if parent:
            QMessageBox.warning(parent, "File Read Error",
                                f"An error occurred while reading {os.path.basename(file_path)}:\n{e}")
        else:
            print(f"File Read Error: An error occurred while reading {os.path.basename(file_path)}:\n{e}")
        return None, None, None

def is_numeric_row(row):
    """
    Determines if a row contains at least two numeric values.
    """
    numeric_count = sum(1 for cell in row if is_float(cell))
    return numeric_count >= max(2, len(row) // 2)

def is_float(value):
    try:
        float(value)
        return True
    except ValueError:
        return False
